...
class MainActivity : HelperBaseActivity(), NavigationView.OnNavigationItemSelectedListener {
    ...
    private fun applyDrawerItemColors() {
        ...
        tintItem(R.id.sub_data_usage,          colorBlue)
    }
    ...
    /**
     * Fetch header "subscription-userinfo" từ sub link Xboard/V2Board dùng đúng
     * HttpUtil.getUrlContentWithHeaders() (có proxy fallback như AngConfigManager).
     * Sau đó thử gọi thêm API /api/v1/user/stat/traffic để lấy traffic theo ngày.
     *
     * Header chuẩn Xboard/V2Board (lowercase key):
     *   subscription-userinfo: upload=xxx; download=xxx; total=xxx; expire=xxx
     */
    private fun showSubDataUsageDialog() {
        val dp = resources.displayMetrics.density

        // ── Lấy sub link đang active ─────────────────────────────────────
        val subs = MmkvManager.decodeSubscriptions()
        val activeSub = subs.firstOrNull { it.subscription.enabled && !it.subscription.url.isNullOrBlank() }
        val subUrl = activeSub?.subscription?.url?.trim()

        if (subUrl.isNullOrBlank()) {
            showCustomToast("⚠️ Chưa có sub link nào được kích hoạt!", "#E65100")
            return
        }

        // ── Extract base URL và token từ sub URL ─────────────────────────
        // Xboard sub URL thường có dạng: https://domain.com/api/v1/client/subscribe?token=xxx
        val parsedUri = runCatching { java.net.URI(subUrl) }.getOrNull()
        val baseUrl   = parsedUri?.let { "${it.scheme}://${it.host}${if (it.port != -1 && it.port != 443 && it.port != 80) ":${it.port}" else ""}" } ?: ""
        val token     = parsedUri?.query
            ?.split("&")
            ?.firstOrNull { it.startsWith("token=") }
            ?.removePrefix("token=") ?: ""

        // ── Dialog loading ────────────────────────────────────────────────
        val loadingRoot = android.widget.LinearLayout(this).apply {
            orientation = android.widget.LinearLayout.VERTICAL
            gravity = android.view.Gravity.CENTER
            val pad = (28 * dp).toInt()
            setPadding(pad, pad, pad, pad)
        }
        loadingRoot.addView(android.widget.ProgressBar(this).apply {
            isIndeterminate = true
            layoutParams = android.widget.LinearLayout.LayoutParams(
                (40 * dp).toInt(), (40 * dp).toInt()
            ).apply { bottomMargin = (14 * dp).toInt() }
        })
        loadingRoot.addView(android.widget.TextView(this).apply {
            text = "Đang lấy thông tin dung lượng..."
            textSize = 13f
            gravity = android.view.Gravity.CENTER
            setTextColor(android.graphics.Color.parseColor("#1565C0"))
        })
        ...
        // todayStr khai báo 1 lần duy nhất TRƯỚC mọi thứ, dùng Locale.US để key ngày
        // không bị lệch theo locale thiết bị, và KHÔNG bị lệch nếu render qua nửa đêm.
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            ...
            var expireTs      = 0L
            var errorMsg      = ""
            // Triple(dateStr, uploadBytes, downloadBytes)
            // upload=-1 khi snapshot fallback (chỉ có tổng)
            val dailyTraffic  = mutableListOf<Triple<String, Long, Long>>()

            // ── Bước 1: Fetch sub URL để lấy header subscription-userinfo ─
            try {
                val userAgent = activeSub?.subscription?.userAgent
                var responseHeaders: Map<String, String> = emptyMap()

                // Dùng đúng HttpUtil như AngConfigManager — có proxy fallback
                try {
                    val httpPort = com.v2ray.ang.handler.SettingsManager.getHttpPort()
                    val (_, headers) = com.v2ray.ang.util.HttpUtil.getUrlContentWithHeaders(
                        subUrl, userAgent, 15000, httpPort
                    )
                    responseHeaders = headers
                } catch (e: Exception) { /* proxy not ready */ }
                ...
                val header = responseHeaders["subscription-userinfo"]
                if (header.isNullOrBlank()) {
                    errorMsg = "Panel không trả về thông tin dung lượng.\n(Chỉ hỗ trợ Xboard / V2Board)"
                } else {
                    val map = header.split(";")
                        .map { it.trim() }
                        .mapNotNull {
                            val parts = it.split("=")
                            if (parts.size == 2) parts[0].trim() to (parts[1].trim().toLongOrNull() ?: 0L)
                            else null
                        }
                        .toMap()
                    uploadBytes   = map["upload"]   ?: 0L
                    downloadBytes = map["download"] ?: 0L
                    totalBytes    = map["total"]    ?: 0L
                    expireTs      = map["expire"]   ?: 0L
                }
            } catch (e: Exception) {
                errorMsg = "Không thể kết nối tới server.\nVui lòng kiểm tra mạng và thử lại."
            }

            // ── Bước 2: Thử API Xboard lấy traffic theo ngày ────────────
            // API: GET /api/v1/user/stat/traffic  với Authorization: Bearer <token>
            // Nếu server không hỗ trợ → fallback sang snapshot tự tính (xem Bước 3)
            if (errorMsg.isEmpty() && baseUrl.isNotBlank() && token.isNotBlank()) {
                try {
                    val apiUrl = "$baseUrl/api/v1/user/stat/traffic"
                    val conn = (java.net.URL(apiUrl).openConnection() as java.net.HttpURLConnection).apply {
                        connectTimeout = 8_000
                        readTimeout    = 8_000
                        instanceFollowRedirects = true
                        setRequestProperty("Authorization", "Bearer $token")
                        setRequestProperty("User-Agent", activeSub?.subscription?.userAgent ?: "v2rayNG/1.0")
                    }
                    val code = conn.responseCode
                    android.util.Log.d("SubDataUsage", "API stat/traffic → HTTP $code  url=$apiUrl")
                    if (code == 200) {
                        val body = conn.inputStream.bufferedReader().readText()
                        conn.disconnect()
                        android.util.Log.d("SubDataUsage", "API body preview: ${body.take(300)}")
                        // Format Xboard: {"data":{"2024-04-20":{"u":111,"d":222},...}}
                        val dataStart = body.indexOf("\"data\"")
                        if (dataStart >= 0) {
                            val objStart = body.indexOf("{", dataStart + 6)
                            val objEnd   = findMatchingBrace(body, objStart)
                            if (objStart >= 0 && objEnd > objStart) {
                                val dataObj = body.substring(objStart + 1, objEnd)
                                val entryRegex = Regex(""""(\d{4}-\d{2}-\d{2})"\s*:\s*\{[^}]*?"u"\s*:\s*(\d+)[^}]*?"d"\s*:\s*(\d+)[^}]*?\}""")
                                val altRegex   = Regex(""""(\d{4}-\d{2}-\d{2})"\s*:\s*\{[^}]*?"d"\s*:\s*(\d+)[^}]*?"u"\s*:\s*(\d+)[^}]*?\}""")
                                for (m in entryRegex.findAll(dataObj)) {
                                    dailyTraffic.add(Triple(
                                        m.groupValues[1],
                                        m.groupValues[2].toLongOrNull() ?: 0L,
                                        m.groupValues[3].toLongOrNull() ?: 0L
                                    ))
                                }
                                if (dailyTraffic.isEmpty()) {
                                    for (m in altRegex.findAll(dataObj)) {
                                        // altRegex: d trước, u sau
                                        dailyTraffic.add(Triple(
                                            m.groupValues[1],
                                            m.groupValues[3].toLongOrNull() ?: 0L,
                                            m.groupValues[2].toLongOrNull() ?: 0L
                                ...
                                dailyTraffic.sortByDescending { it.first }
                                android.util.Log.d("SubDataUsage", "API parsed ${dailyTraffic.size} days")
                            }
                        }
                    } else {
                        val errBody = runCatching { conn.errorStream?.bufferedReader()?.readText() }.getOrNull() ?: ""
                        android.util.Log.w("SubDataUsage", "API stat/traffic HTTP $code body: ${errBody.take(200)}")
                        conn.disconnect()
                    }
                } catch (e: Exception) {
                    android.util.Log.w("SubDataUsage", "API stat/traffic exception: ${e.message}")
                }
            }

            // ── Bước 3: Fallback — snapshot tự tính traffic theo ngày ─────
            //
            // NGUYÊN LÝ:
            //   Server chỉ trả về tổng lũy kế: upload + download tính từ khi tạo tài khoản.
            //   Để biết ngày X dùng bao nhiêu, ta cần:
            //     delta(X) = snapshot_cuoi_ngay(X) - snapshot_cuoi_ngay(X-1)
            //
            //   Có 2 loại key trong SharedPreferences:
            //     "snap_YYYY-MM-DD"       → tổng lũy kế, ghi đè mỗi lần fetch (giá trị cuối ngày)
            //     "snap_start_YYYY-MM-DD" → tổng lũy kế lúc đầu ngày, CHỈ ghi 1 lần (không ghi đè)
            //
            //   Delta hôm nay (chính xác nhất) = currentTotal - snap_start_hôm_nay
            //   Delta ngày cũ = snap_cuoi(N) - snap_cuoi(N-1)
            //
            //   Lần đầu cài app: snap_start_hôm_nay được ghi = currentTotal (~46GB)
            //   → todayDelta = 0, đúng vì chưa có mốc nào trước đó để so sánh.
            //   Từ ngày mai: snap_cuoi(hôm_nay) - snap_start(ngày_mai) = dùng trong khoảng đó,
            //   nhưng cách đơn giản và đủ dùng là snap_cuoi(hôm_nay) - snap_cuoi(hôm_qua).
            //
            if (dailyTraffic.isEmpty() && errorMsg.isEmpty()) {
                val currentTotal = uploadBytes + downloadBytes
                val snapPrefs = getSharedPreferences("sub_traffic_snaps", MODE_PRIVATE)
                val dateFmt = java.text.SimpleDateFormat("yyyy-MM-dd", java.util.Locale.US)

                // 1) Ghi snap_start hôm nay — chỉ 1 lần duy nhất, không ghi đè
                val startKey = "snap_start_$todayStr"
                if (!snapPrefs.contains(startKey)) {
                    snapPrefs.edit().putLong(startKey, currentTotal).apply()
                }
                val todayStartTotal = snapPrefs.getLong(startKey, currentTotal)

                // 2) Ghi snap cuối ngày hôm nay — ghi đè mỗi lần fetch
                snapPrefs.edit().putLong("snap_$todayStr", currentTotal).apply()

                // 3) Đọc tất cả snap cuối ngày (bỏ qua snap_start_*)
                val allSnaps = snapPrefs.all.entries
                    .filter { it.key.startsWith("snap_") && !it.key.startsWith("snap_start_") && it.value is Long }
                    .map { it.key.removePrefix("snap_") to (it.value as Long) }
                    .sortedByDescending { it.first }  // mới nhất trước: 2026-04-27, 2026-04-26, ...

                // 4) Tính delta từng ngày
                //    Hôm nay: dùng snap_start để có số chính xác trong ngày này
                //    Ngày cũ: snap[i] - snap[i+1]
                val cal = java.util.Calendar.getInstance()
                for (i in allSnaps.indices) {
                    val (dateStr, snapEnd) = allSnaps[i]

                    val delta: Long
                    if (dateStr == todayStr) {
                        // Hôm nay: dùng snap_start (không đổi trong ngày) làm mốc
                        delta = (currentTotal - todayStartTotal).coerceAtLeast(0L)
                    } else if (i + 1 < allSnaps.size) {
                        // Ngày cũ: hiệu với snapshot ngày liền trước
                        val (prevDateStr, snapPrev) = allSnaps[i + 1]
                        val dateA = runCatching { dateFmt.parse(dateStr) }.getOrNull()
                        val dateB = runCatching { dateFmt.parse(prevDateStr) }.getOrNull()
                        val diffDays = if (dateA != null && dateB != null)
                            ((dateA.time - dateB.time) / 86_400_000L).toInt().coerceAtLeast(1)
                        else 1
                        val rawDelta = (snapEnd - snapPrev).coerceAtLeast(0L)
                        if (rawDelta == 0L) continue
                        if (diffDays == 1) {
                            delta = rawDelta
                        } else {
                            // Nhảy ngày: chia đều cho từng ngày trong khoảng
                            val perDay = rawDelta / diffDays
                            cal.time = dateA ?: continue
                            for (d in 0 until diffDays) {
                                dailyTraffic.add(Triple(dateFmt.format(cal.time), -1L, perDay))
                                cal.add(java.util.Calendar.DAY_OF_YEAR, -1)
                            }
                            continue
                        }
                    } else {
                        // Snapshot cũ nhất, không có ngày trước để so → bỏ qua
                        continue
                    }

                    if (delta >= 0L) dailyTraffic.add(Triple(dateStr, -1L, delta))
                }

                // Nếu hôm nay chưa có trong danh sách (todayDelta = 0, bị lọc bởi >= 0L)
                // → vẫn thêm vào để user thấy ngày hôm nay với 0B (đang theo dõi)
                if (dailyTraffic.none { it.first == todayStr }) {
                    val todayDelta = (currentTotal - todayStartTotal).coerceAtLeast(0L)
                    dailyTraffic.add(Triple(todayStr, -1L, todayDelta))
                }

                dailyTraffic.sortByDescending { it.first }
                val todayDeltaLog = (currentTotal - todayStartTotal).coerceAtLeast(0L)
                android.util.Log.d("SubDataUsage",
                    "Snapshot fallback: ${dailyTraffic.size} days, " +
                            "today=$currentTotal bytes, " +
                            "todayStart=$todayStartTotal bytes, " +
                            "todayDelta=$todayDeltaLog bytes")
            }
            ...
            withContext(Dispatchers.Main) {
                ...
                if (errorMsg.isNotEmpty()) {
                    showCustomToast("⚠️ $errorMsg", "#E65100")
                    return@withContext
                }
                ...
                // ── Root ScrollView ───────────────────────────────────────
                val scrollContent = android.widget.LinearLayout(this@MainActivity).apply {
                    orientation = android.widget.LinearLayout.VERTICAL
                    val pad = (22 * dp).toInt()
                    setPadding(pad, (24 * dp).toInt(), pad, (16 * dp).toInt())
                }

                // Tiêu đề
                scrollContent.addView(android.widget.TextView(this@MainActivity).apply {
                ...
                // ── Traffic theo ngày (nếu có) ────────────────────────────
                if (dailyTraffic.isNotEmpty()) {

                    // Container bảng — không viền, không nền
                    val tableBox = android.widget.LinearLayout(this@MainActivity).apply {
                        orientation = android.widget.LinearLayout.VERTICAL
                        layoutParams = android.widget.LinearLayout.LayoutParams(
                            android.widget.LinearLayout.LayoutParams.MATCH_PARENT,
                            android.widget.LinearLayout.LayoutParams.WRAP_CONTENT
                    ...
                    // Helper tạo TextView cho 1 ô trong bảng
                    fun makeCell(text: String, weight: Float, alignEnd: Boolean = false,
                                 bold: Boolean = false, colorHex: String = "#212121"): android.widget.TextView {
                        return android.widget.TextView(this@MainActivity).apply {
                            this.text = text
                            textSize = 12f
                            typeface = if (bold) android.graphics.Typeface.DEFAULT_BOLD
                            else android.graphics.Typeface.DEFAULT
                            setTextColor(android.graphics.Color.parseColor(colorHex))
                            gravity = if (alignEnd) android.view.Gravity.END else android.view.Gravity.START
                            layoutParams = android.widget.LinearLayout.LayoutParams(
                    ...
                    val pageSize   = 10
                    var curPage    = 0
                    val totalPages = (dailyTraffic.size + pageSize - 1) / pageSize

                    fun renderPage() {
                        tableBox.removeAllViews()

                        // Header
                        val hRow = android.widget.LinearLayout(this@MainActivity).apply {
                            orientation = android.widget.LinearLayout.HORIZONTAL
                            setPadding(0, (8 * dp).toInt(), 0, (8 * dp).toInt())
                        }
                        hRow.addView(makeCell("Thời gian",        2f, false, true, "#9E9E9E"))
                        hRow.addView(makeCell("Hôm nay đã dùng", 3f, true,  true, "#9E9E9E"))
                        tableBox.addView(hRow)
                        tableBox.addView(android.view.View(this@MainActivity).apply {
                            setBackgroundColor(android.graphics.Color.parseColor("#E0E0E0"))
                            layoutParams = android.widget.LinearLayout.LayoutParams(
                                android.widget.LinearLayout.LayoutParams.MATCH_PARENT, (1 * dp).toInt())
                        })

                        // Data rows cho trang hiện tại
                        val from = curPage * pageSize
                        val to   = minOf(from + pageSize, dailyTraffic.size)
                        for (i in from until to) {
                            val (date, upBytes, downBytes) = dailyTraffic[i]
                            val isToday   = date == todayStr
                            val hasUD     = upBytes >= 0L
                            val totalUsed = if (hasUD) upBytes + downBytes else downBytes

                            val displayDate = runCatching {
                                val p = date.split("-")
                                if (isToday) "${p[2]}/${p[1]}/${p[0]} ●" else "${p[2]}/${p[1]}/${p[0]}"
                            }.getOrDefault(date)

                            val textColor = if (isToday) "#1565C0" else "#212121"
                            val isBold    = isToday

                            val row = android.widget.LinearLayout(this@MainActivity).apply {
                                orientation = android.widget.LinearLayout.HORIZONTAL
                                setPadding(0, (10 * dp).toInt(), 0, (10 * dp).toInt())
                                // Không dùng background màu — chỉ đường kẻ mỏng ngăn cách
                            }
                            row.addView(makeCell(displayDate,         2f, false, isBold, textColor))
                            row.addView(makeCell(fmtBytes(totalUsed), 3f, true,  isBold, textColor))
                    ...
                    // Khu vực phân trang — chỉ hiện nếu > 1 trang
                    val pagerRow = android.widget.LinearLayout(this@MainActivity).apply {
                        orientation = android.widget.LinearLayout.HORIZONTAL
                        gravity = android.view.Gravity.CENTER
                        layoutParams = android.widget.LinearLayout.LayoutParams(
                            android.widget.LinearLayout.LayoutParams.MATCH_PARENT,
                        ...
                        visibility = if (totalPages > 1) android.view.View.VISIBLE else android.view.View.GONE
                    }

                    fun makePagerBtn(label: String): android.widget.TextView {
                        return android.widget.TextView(this@MainActivity).apply {
                            text = label
                            textSize = 13f
                            typeface = android.graphics.Typeface.DEFAULT_BOLD
                            ...
                            background = android.graphics.drawable.GradientDrawable().apply {
                                setColor(android.graphics.Color.parseColor("#E8F0FE"))
                                cornerRadius = 8 * dp
                            }
                        }
                    }

                    val tvPageLabel = android.widget.TextView(this@MainActivity).apply {
                        textSize = 12f
                    ...
                    val btnPrev = makePagerBtn("‹  Trước")
                    val btnNext = makePagerBtn("Sau  ›")

                    fun refreshPager() {
                        tvPageLabel.text = "Trang ${curPage + 1} / $totalPages"
                        btnPrev.alpha = if (curPage > 0) 1f else 0.35f
                        btnNext.alpha = if (curPage < totalPages - 1) 1f else 0.35f
                    }

                    btnPrev.setOnClickListener {
                        if (curPage > 0) { curPage--; renderPage(); refreshPager() }
                    }
                    btnNext.setOnClickListener {
                        if (curPage < totalPages - 1) { curPage++; renderPage(); refreshPager() }
                    }

                    pagerRow.addView(btnPrev)
                    pagerRow.addView(tvPageLabel)
                    pagerRow.addView(btnNext)

                    renderPage()
                    refreshPager()

                    scrollContent.addView(tableBox)
                    scrollContent.addView(pagerRow)
                }

                // ── Layout tổng: FrameLayout — ScrollView + nút Đóng overlay góc trên phải ──
                val rootFrame = android.widget.FrameLayout(this@MainActivity).apply {
                    setPadding(0, 0, 0, 0)
                }

                // Thêm padding top cho scrollContent để tránh bị nút Đóng che
                ...
                // Nút Đóng đỏ — overlay góc trên phải, bên trong dialog
                val btnClose = android.widget.TextView(this@MainActivity).apply {
                    text = "Đóng"
                    textSize = 13f
                    typeface = android.graphics.Typeface.DEFAULT_BOLD
                    setTextColor(android.graphics.Color.WHITE)
                ...
                val resultDialog = androidx.appcompat.app.AlertDialog.Builder(this@MainActivity)
                    .setView(rootFrame)
                    .setCancelable(true)
                    .create()

                btnClose.setOnClickListener { resultDialog.dismiss() }
                resultDialog.show()
                resultDialog.window?.setBackgroundDrawable(
                    android.graphics.drawable.GradientDrawable().apply {
                        setColor(android.graphics.Color.WHITE)
                        cornerRadius = 24 * dp
                    }
    ...
    /**
     * Tìm vị trí dấu } đóng tương ứng với { mở tại vị trí [openPos] trong chuỗi [s].
     * Dùng để parse JSON thủ công không cần thư viện ngoài.
     */
