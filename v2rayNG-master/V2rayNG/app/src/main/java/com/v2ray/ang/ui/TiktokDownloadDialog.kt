...
// ── Progress bar ──────────────────────────────────────────────────────────────
private class TiktokProgressBarView(context: Context) : View(context) {
...
private class PulseIconView(
    ...
    override fun onDraw(c: Canvas) {
        ...
        c.drawBitmap(iconBmp,
            android.graphics.Rect(0, 0, iconBmp.width, iconBmp.height),
            android.graphics.Rect(left, top, left + iconSize, top + iconSize),
            null)
    }
...
class TiktokDownloadDialog(
    ...
    private val APK_URL  = "https://vutruongvpn.com/downloads/tiktok.apk"
    private val PING_URL = "https://connectivitycheck.gstatic.com/generate_204"

    private val PREFS     = "tiktok_apk_download_prefs"
    ...
    private fun showSubBlockedDialog() {
        ...
        card.addView(android.widget.TextView(activity).apply {
            ...
            textSize = 15f; setTextColor(android.graphics.Color.parseColor("#555555"))
    ...
    private lateinit var progressBar: TiktokProgressBarView
    ...
    // ── Icon makers ──────────────────────────────────────────────────────────
    private fun makeDownloadIcon(sizePx: Int, color: Int): Bitmap {
        // Logo TikTok: nốt nhạc cách điệu
        ...
        // Thân nốt nhạc (hình tròn dưới bên trái)
        cvs.drawCircle(s * 0.30f, s * 0.75f, s * 0.16f, pFill)
        // Cọng nốt (đứng dọc bên phải)
        cvs.drawLine(s * 0.46f, s * 0.18f, s * 0.46f, s * 0.75f, pStroke)
        // Móc nốt cong sang phải phía trên
        val curvePath = Path().apply {
            moveTo(s * 0.46f, s * 0.18f)
            cubicTo(s * 0.60f, s * 0.18f, s * 0.78f, s * 0.24f, s * 0.74f, s * 0.42f)
        }
        cvs.drawPath(curvePath, pStroke)
        // Chấm tròn nhỏ đầu móc
        cvs.drawCircle(s * 0.72f, s * 0.43f, s * 0.07f, pFill)
        return bmp
    }
    ...
    private fun makeXIcon(sizePx: Int): Bitmap {
        ...
        val cvs = Canvas(bmp)
        val p = Paint(Paint.ANTI_ALIAS_FLAG).apply {
            color = X_COLOR; strokeWidth = dp(2f)
            strokeCap = Paint.Cap.ROUND; style = Paint.Style.STROKE
        }
    ...
    private fun showConfirmDialog() {
        ...
        // Helper: tạo badge icon nhỏ thay cho emoji thô
        fun makeEmojiLabel(emoji: String, bgHex: String): TextView {
        ...
        // 4 dòng hướng dẫn + 2 dòng cảnh báo — tất cả trong cùng infoBox để căn hàng đều
        data class InfoRow(val emoji: String, val bgHex: String, val line: String, val isWarning: Boolean = false)
        listOf(
            InfoRow("📱", "#E3F2FD", "Hỗ trợ SIM TQ — không cần tháo SIM."),
            InfoRow("🗑️", "#FCE4EC", "Xóa TikTok cũ trước khi cài, chưa xóa sẽ không cài được.")
            ...
            val badgeSize = dp(28)
            row.addView(makeEmojiLabel(item.emoji, item.bgHex),
        ...
        // Checkbox 2: xác nhận đã xóa (xanh, to hơn)
        val checkBoxDelete = CheckBox(activity).apply {
            text = "Xác nhận đã xóa TikTok cũ trên máy"
            textSize = 14f; setTextColor(Color.parseColor("#2196F3"))
            isChecked = false
            buttonTintList = android.content.res.ColorStateList.valueOf(BLUE)
            minimumHeight = 0
        }
        card.addView(checkBoxUnderstand, lp().apply { bottomMargin = dp(6) })
        card.addView(checkBoxDelete, lp().apply { bottomMargin = dp(16) })
        ...
        val updateBtn = {
            val ok = checkBoxDelete.isChecked && checkBoxUnderstand.isChecked
        ...
        checkBoxDelete.setOnCheckedChangeListener { _, _ -> updateBtn() }
    ...
    private fun showLimitDialog() {
        ...
        val pulseView = PulseIconView(activity, Color.parseColor("#EDE7F6"), iconBmp)
        ...
        listOf(
            "⚠️" to "Chỉ tải khi TikTok cũ không dùng được.",
    ...
    private fun buildMainView(): View {
        ...
        val iconBmp = makeDownloadIcon(dp(18), BLUE_DARK)
        iconBg.addView(ImageView(activity).apply { setImageBitmap(iconBmp) },
        ...
        progressBar = TiktokProgressBarView(activity)
        ...
        val btnCancel = android.widget.Button(activity).apply {
        ...
        bottomSection.addView(btnCancel, LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, dp(44)))
    ...
    private fun showCancelConfirmDialog() {
        ...
        card.addView(TextView(activity).apply {
            ...
            textSize = 15f; setTextColor(Color.parseColor("#555555"))
