...
// ── Progress bar ──────────────────────────────────────────────────────────────
private class VpnUpdateProgressBarView(context: Context) : View(context) {
...
// ── Pulse icon view ───────────────────────────────────────────────────────────
private class VpnPulseIconView(
    context: Context,
    ...
    override fun onDraw(c: Canvas) {
        ...
        c.drawBitmap(iconBmp,
            ...
            android.graphics.Rect(left, top, left + iconSize, top + iconSize), null)
    }
...
class VpnUpdateDownloadDialog(
    ...
    private val APK_URL  = "https://vutruongvpn.com/downloads/yumvpn.apk"
    private val PING_URL = "https://connectivitycheck.gstatic.com/generate_204"

    private val PREFS     = "vpn_update_download_prefs"
    ...
    private lateinit var progressBar: VpnUpdateProgressBarView
    ...
    /** Icon VPN: shield/khiên */
    private fun makeVpnIcon(sizePx: Int, color: Int): Bitmap {
        ...
        val path = Path().apply {
            ...
            lineTo(s * 0.82f, s * 0.55f)
            cubicTo(s * 0.82f, s * 0.75f, s * 0.65f, s * 0.87f, s * 0.50f, s * 0.92f)
            cubicTo(s * 0.35f, s * 0.87f, s * 0.18f, s * 0.75f, s * 0.18f, s * 0.55f)
            lineTo(s * 0.18f, s * 0.28f)
            close()
        }
        cvs.drawPath(path, p)
        // Check mark di trong
    ...
    // ── Dialog xác nhận (hướng dẫn + checkbox) ───────────────────────────────
    private fun showConfirmDialog() {
        ...
        // Tất cả dòng lưu ý + hướng dẫn cài gộp chung, không khung riêng
        ...
        data class InfoRow(val emoji: String, val bgHex: String, val line: String)
        listOf(
            InfoRow("✓", "#F5F5F7","Phiên bản mới tối ưu hiệu suất và giảm lỗi kết nối."),
            InfoRow("🛠", "#F5F5F7","Cập nhật ngay để nhận các bản sửa lỗi và bảo mật mới nhất.")
            ...
            row.addView(makeEmojiBadge(item.emoji, item.bgHex),
                LinearLayout.LayoutParams(badgeSize, badgeSize).apply { rightMargin = dp(10) })
            row.addView(TextView(activity).apply {
                text = item.line; textSize = 13f
                setTextColor(Color.BLACK)
                isSingleLine = false
            }, LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.WRAP_CONTENT, 1f))
            infoBox.addView(row, lp().apply { topMargin = if (i > 0) dp(10) else 0 })
        }
        ...
        card.addView(btnDownload, LinearLayout.LayoutParams(
    ...
    private fun showVpnBlockedDialog(pingFailed: Boolean) {
        ...
        val iconView = object : android.view.View(activity) {
            ...
            override fun onDraw(canvas: android.graphics.Canvas) {
                ...
                // Scan line bên trong shield
                val sc = android.graphics.Canvas(android.graphics.Bitmap.createBitmap(width, height, android.graphics.Bitmap.Config.ARGB_8888))
                sc.clipPath(shieldPath)
                sc.drawRect(0f, scanY - dp(2f), width.toFloat(), scanY + dp(2f),
    ...
    private fun showLimitDialog() {
        ...
        val pulseView = VpnPulseIconView(activity, Color.parseColor("#EDE7F6"), iconBmp)
        ...
        listOf(
            "⚠️" to "Chỉ tải khi admin yêu cầu hoặc app cũ bị lỗi.",
    ...
    private fun buildMainView(): View {
        ...
        iconBg.addView(ImageView(activity).apply { setImageBitmap(makeVpnIcon(dp(18), BLUE_DARK)) },
        ...
        progressBar = VpnUpdateProgressBarView(activity)
    ...
    private fun startDownload() {
        ...
        pollJob = activity.lifecycleScope.launch(Dispatchers.IO) {
            try {
                ...
                val effectiveTotal = if (total > 0) total else 50L * 1024 * 1024
                    ...
                    response.body!!.byteStream().use { input ->
                        ...
                        while (true) {
                            ...
                            if (now - lastUi >= 100) {
                                ...
                                val totalMb = if (total > 0) "%.0f MB".format(total / 1_048_576f) else "~50 MB"
