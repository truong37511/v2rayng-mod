...
object MmkvManager {
    ...
    fun decodeAllServerList(): MutableList<String> {
        ...
        // Always include servers from DEFAULT_SUBSCRIPTION_ID (ungrouped/manually added servers)
        allServers.addAll(decodeServerList(DEFAULT_SUBSCRIPTION_ID))

        // Add servers from all subscriptions (excluding DEFAULT_SUBSCRIPTION_ID to avoid duplicates)
        subsList.forEach { guid ->
            if (guid != DEFAULT_SUBSCRIPTION_ID) {
    ...
    /**
     * Removes all server configurations without affecting settings or subscriptions.
     *
    ...
    fun removeAllServer(): Int {
        ...
        // Remove all profiles and affiliation data
        ...
        // Clear all per-subscription server lists (SUB_SERVERS_* keys)
