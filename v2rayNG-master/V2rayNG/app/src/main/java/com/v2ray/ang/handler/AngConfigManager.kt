...
object AngConfigManager {
    ...
    private fun parseBatchConfig(servers: String?, subid: String, append: Boolean): Int {
        try {
            ...
            Log.d(AppConfig.TAG, "DEBUG parseBatchConfig: subid=$subid filter='${subItem?.filter}' lines=${servers.lines().size}")
            ...
            servers.lines()
                ...
                .forEach {
                    ...
                    if (config != null) {
                        ...
                        Log.d(AppConfig.TAG, "DEBUG: line NOT parsed: ${it.trim().take(80)}")
                    }
                }
            Log.d(AppConfig.TAG, "DEBUG parseBatchConfig: parsed ${configs.size} configs total")
    ...
    private fun batchSaveConfigs(configs: List<ProfileItem>, subid: String): Map<String, ProfileItem> {
        ...
        // Read serverList once (use same subid so it maps to DEFAULT_SUBSCRIPTION_ID when blank)
        ...
        // Write serverList once (use same subid — MmkvManager maps blank to DEFAULT_SUBSCRIPTION_ID)
    ...
    fun updateConfigViaSub(it: SubscriptionCache): SubscriptionUpdateResult {
        try {
            ...
            if (configText.isEmpty()) {
                Log.e(AppConfig.TAG, "DEBUG: configText is EMPTY after both fetch attempts")
                return SubscriptionUpdateResult(failureCount = 1)
            }
            ...
            if (count > 0) {
                ...
                Log.e(AppConfig.TAG, "DEBUG: Got configText but count=0 — no server parsed. Check format or filter.")
                return SubscriptionUpdateResult(failureCount = 1)
            }
        } catch (e: Exception) {
